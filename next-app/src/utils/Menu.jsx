...
import faIcons from "@/helpers/icons";

export default function Menu(props) {
  const dict = props.dict;
  const pages = dict.general?.pages || [];

  const menuType = 2 //hard codeded - no configuration for now
  ...
  function DropdownMenu() {
    return (
      ...
      >
        ...
        >
          <div
            className={`flex items-center gap-2 ${isMenuOpen ? 'hidden' : 'block'
              }`}
          >
  ...
  function IconMenu() {
    return (
      <div
        ref={iconMenuRef}
        style={{ touchAction: 'none' }}
        className=" z-0 fixed bottom-0 flex flex-row items-center justify-center gap-6 py-2 font-display text-white text-lg bg-black w-full border-t-2 border-solid border-white md:hidden overflow-hidden"
      >
        <Link key={0} href="/">
          <button className="flex flex-col font-header text-xs items-center justify-center relative">
            <FontAwesomeIcon alt="Home" icon={faIcons["faHouse"]} size="2x" />
            <span>HOME</span>
          </button>
        </Link>
        {pages.map((item, index) => (
          <Link key={index + 1} href={`${item.link}`}>
            <button className="flex flex-col font-header text-xs items-center justify-center relative">
              <FontAwesomeIcon alt={item} icon={faIcons[item.icon]} size="2x" />
              <span>{item.name}</span>
            </button>
          </Link>
        ))}
